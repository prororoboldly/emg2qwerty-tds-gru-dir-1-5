...
@dataclass
class ChannelSubset:
    """Select the first ``num_channels`` electrode channels from a spectrogram.

    Input must be of shape (T, bands, C, freq) — i.e. after ``LogSpectrogram``.
    Returns a tensor of shape (T, bands, num_channels, freq).

    Args:
        num_channels (int): Number of electrode channels to keep per band.
    """
...
@dataclass
class Downsample:
    """Downsample the raw EMG signal by selecting every ``factor``-th sample,
    simulating a lower acquisition sampling rate (2000 Hz / factor).

    Input must be of shape (T, ...) — i.e. after ``ToTensor`` and before
    ``LogSpectrogram``. Returns a tensor of shape (T // factor, ...).

    Args:
        factor (int): Downsampling factor. E.g. factor=4 simulates 500 Hz.
    """
