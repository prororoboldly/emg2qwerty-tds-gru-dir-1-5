...
class TDSConvCTCModule(pl.LightningModule):
    ...
    def __init__(
        ...
        self.model = nn.Sequential(
            ...
            SpectrogramNorm(channels=self.NUM_BANDS * num_electrode_channels),
...
class CNNGRUCTCModule(pl.LightningModule):
    ...
    def __init__(
        ...
        cnn_frontend = CNNFrontend(
            num_bands = self.NUM_BANDS,
            electrode_channels = self.ELECTRODE_CHANNELS,
            conv_channels = conv_channels,
        )
        gru_encoder = GRUEncoder(
            num_features=cnn_frontend.out_features,
            hidden_size=hidden_size,
            num_layers=num_layers,
            dropout=dropout,
            bidirectional=bidirectional,
...
class CNNCTCModule(pl.LightningModule):
    ...
    def __init__(
        ...
        self.model = nn.Sequential(
            SpectrogramNorm(channels=self.NUM_BANDS * self.ELECTRODE_CHANNELS),
            cnn_frontend,
            nn.LayerNorm(cnn_frontend.out_features),
            nn.Linear(cnn_frontend.out_features, charset().num_classes),
            nn.LogSoftmax(dim=-1),
        )
...
class CNNLSTMCTCModule(pl.LightningModule):
    ...
    def __init__(
        ...
        self.model = nn.Sequential(
            ...
            cnn_frontend,
            lstm_encoder,
            nn.LayerNorm(lstm_encoder.out_features),
